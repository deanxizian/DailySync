...
| **Sync Garmin CN to COROS CN** | 手动 | 只执行佳明到高驰日常同步 |
| **Sync COROS CN to Garmin CN** | 手动 | 只执行高驰到佳明日常同步 |
| **Sync Garmin CN and COROS CN** | 手动、每 6 小时 | 按顺序执行两个日常同步方向 |
...
双向日常 Action 在北京时间 **04:00、10:00、16:00、22:00** 运行，并在同一次任务中依次执行佳明到高驰、高驰到佳明，避免同刻触发的两个任务互相替换。全部高驰相关任务使用同一个 GitHub Actions 并发组，避免迁移和同步同时上传。原佳明 Action 保持独立，继续在北京时间 **02:00、08:00、14:00、20:00** 运行。
