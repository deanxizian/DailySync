...
const family = sport => ({ running: 'running', cycling: 'cycling', swimming: 'swimming', hiking: 'hiking',
    walking: 'walking', training: 'strength', fitnessEquipment: 'cardio', rockClimbing: 'climbing' }[sport] || sport);
...
export function decodeEvidence(bytes) {
    ...
    return { sha256: hash(bytes), device, records: recordHash, start, sport: family(session.sport),
