...
export function gpxToTcx(bytes: Buffer, activity: Activity): Buffer {
    ...
    if (!Number.isFinite(activity.start) || activity.start <= 0 || Math.abs(firstPoint.time - activity.start) > 60000) {
    ...
    const timeShift = activity.start - firstPoint.time;
    ...
    for (const segment of segments) {
        for (let index = 0; index < segment.length; index++) {
            ...
            if (index) computedDistance += haversine(segment[index - 1]!, point);
            point.distance = computedDistance;
        }
    }
    ...
    const elapsed = (lastPoint.time - firstPoint.time) / 1000;
    ...
    const start = new Date(firstPoint.time).toISOString();
    const document = {
...
export function tcxEvidence(bytes: Buffer, expectedSport?: string): Evidence {
    try {
        ...
        const sport = activity?.Sport === 'Biking' ? 'cycling' : activity?.Sport === 'Running' ? 'running' :
            activity?.Sport === 'Other' && expectedSport ? expectedSport : undefined;
        ...
        if (error instanceof SyncError) throw error;
        throw new SyncError('TCX_INVALID', 'TCX must contain one complete activity with a recognized sport.');
    }
}
